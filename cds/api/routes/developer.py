...
def get_developers(session: Session = Depends(get_session)):
    with session:
...
def get_developer(dev_id: int, session: Session = Depends(get_session)):
    with session:
        stmt = select(Developer).where(Developer.id == dev_id)
        results = session.exec(stmt).one()
        return results

@dev_route.get('/{dev_id}/challenge/', response_model=list[DevChallengeScoreRead])
def get_all_dev_scores(dev_id: int, session: Session = Depends(get_session)):
    with session:
        stmt = (
...
def get_one_dev_score(dev_id: int, challenge_id: int, session: Session = Depends(get_session)):
    with session:
        stmt = (
            select(DevChallengeScore)
            .join(Developer)
